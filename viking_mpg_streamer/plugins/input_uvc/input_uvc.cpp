...
int input_init(input_parameter *param, int id)
{
    char *dev = NULL;
    char *s = NULL;
    int width = 640, height = 480, fps = 30, format = V4L2_PIX_FMT_MJPEG, i;
...
void *cam_thread(void *arg)
{
    ...
    while (!pglobal->stop)
    {
        ...
        CTestMonitor::updateTimeSinceLastCameraFrame(pcontext->id);
        ...
        if (g_pFrameGrinder->safeGetFreeFrame(&pFrame))
        {
            ...
            if (!pFrame->m_frame.empty())
            {
                if (0 == pcontext->id)
                {
                    ...
                    g_pFrameGrinder->safeAddTailAndPurgeOlder(pFrame, CVideoFrame::FRAME_QUEUE_CAM1);
                }
                else
                {
                    CTestMonitor::getTicks(&pFrame->m_timeAddedToQueue[(int) CVideoFrame::FRAME_QUEUE_CAM2]);
                    g_pFrameGrinder->safeAddTailAndPurgeOlder(pFrame, CVideoFrame::FRAME_QUEUE_CAM2);
                }
            }
            else
            {
            ...
            if (pFrame != NULL)
            {
                pFrame->m_targetInfo.setTimeSinceLastCameraFrameMilliseconds(pcontext->id, CTestMonitor::getTimeSinceLastCameraFrame(pcontext->id));
            }
         }
